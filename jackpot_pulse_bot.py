...
try:
    ...
    print("⚠️ Основний файл пошкоджено. Відновлення з резервної копії...")
    if os.path.exists(DATA_FILE + '.bak'):
        with open(DATA_FILE + '.bak', 'r') as f:
            users_data = json.load(f)
            users_data = {int(k): v for k, v in users_data.items()}
    else:
        users_data = {}
...
main_keyboard.row('📊 Мій профіль', '🎟 Промокод')
main_keyboard.row('👯 Запросити друга', '⭐️ Топ 5 гравців')
main_keyboard.row('📊 Баланс', '💸 Вивести')
...
def daily_bonus(message):
    ...
    if not check_subscriptions(user_id):
        bot.send_message(message.chat.id, "⚠️ Спершу підпишись на всі спонсорські канали.", reply_markup=main_keyboard)
        return

    now = int(time.time())
    if now - users_data[user_id].get('last_bonus', 0) < 86400:
        bot.send_message(message.chat.id, "🕐 Фарт вже отримано сьогодні. Спробуй завтра!", reply_markup=main_keyboard)
        return

    bonus = random.randint(15, 100)
    users_data[user_id]['pulse'] += bonus
    users_data[user_id]['last_bonus'] = now
    users_data[user_id]['streak'] += 1
    save_data()
    bot.send_message(message.chat.id, f"🎉 Ви отримали {bonus} PulseCoins!", reply_markup=main_keyboard)

@bot.message_handler(func=lambda m: m.text == '🃏 Фарт-картка')
def fart_card(message):
    user_id = message.from_user.id
    if not check_subscriptions(user_id):
        bot.send_message(message.chat.id, "⚠️ Підпишись на всі спонсорські канали!", reply_markup=main_keyboard)
        return

    now = int(time.time())
    if now - users_data[user_id].get('last_card', 0) < 86400:
        bot.send_message(message.chat.id, "⏳ Фарт-картку можна відкривати лише раз на добу.", reply_markup=main_keyboard)
        return

    users_data[user_id]['last_card'] = now
    outcomes = [
        ("💰 +50 PulseCoins!", 50),
        ("🎟 Квиток на розіграш!", 0),
        (f"🤣 {random.choice(jokes)}", 0),
        ("🤷‍♂️ Нічого не випало цього разу.", 0)
    ]
    result, coins = random.choice(outcomes)
    if "Квиток" in result:
        users_data[user_id]['tickets'] += 1
    if coins > 0:
        users_data[user_id]['pulse'] += coins
    save_data()
    bot.send_message(message.chat.id, f"🃏 Фарт-картка: {result}", reply_markup=main_keyboard)

@bot.message_handler(func=lambda m: m.text == '🎟 Промокод')
def promo_code(message):
    msg = bot.send_message(message.chat.id, "🔑 Введи свій промокод:")
...
def process_promo_code(message):
    ...
    if code in promocodes and promocodes[code]['activations'] > 0:
        users_data[user_id]['lucky'] += promocodes[code]['value']
        promocodes[code]['activations'] -= 1
        save_data()
        bot.send_message(message.chat.id, f"✅ Ви отримали {promocodes[code]['value']} LuckyTokens!", reply_markup=main_keyboard)
    else:
        bot.send_message(message.chat.id, "❌ Промокод недійсний або вичерпано.", reply_markup=main_keyboard)

# Додамо кнопку Баланс з підкнопкою Вивести
...
def back_to_main(message):
    bot.send_message(message.chat.id, "🔙 Повернення до головного меню", reply_markup=main_keyboard)

@bot.message_handler(func=lambda m: m.text == '💸 Вивести')
def request_withdraw_requisites(message):
    user_id = message.from_user.id
    balance = users_data.get(user_id, {}).get('balance', 0)
    if balance < 250:
        bot.send_message(message.chat.id, "❌ Для виведення потрібно мінімум 250 LuckyTokens", reply_markup=main_keyboard)
        return

    msg = bot.send_message(message.chat.id, "✏️ Введіть реквізити для виводу (наприклад, номер картки або Mono/PayPal/...):")
...
def process_withdraw_request(message):
    ...
    balance = users_data.get(user_id, {}).get('balance', 0)
    if balance < 250:
        bot.send_message(message.chat.id, "❌ Недостатньо балансу.", reply_markup=main_keyboard)
        return
    ...
    bot.send_message(message.chat.id, "✅ Заявка на виведення прийнята. Очікуйте до 48 годин.", reply_markup=main_keyboard)
    bot.send_message(ADMIN_ID, f"📥 Нова заявка на виведення від {message.from_user.first_name} (@{message.from_user.username}):\n\n" +
                     f"Сума: 250 LuckyTokens\nРеквізити: {requisites}\nЧас: {time.ctime()}")
...
# При ініціалізації нового користувача:
def reset_user(user_id):
    users_data[user_id] = {
        'pulse': 0,  # PulseCoins для бонусів
        'lucky': 0,  # LuckyTokens для виведення
        'last_bonus': 0,
        'last_card': 0,
        'streak': 0,
        'referrals': 0,
...
# ✅ Додано показ обох валют у профілі та надійне збереження бази з резервною копією
...
def my_profile(message):
    ...
    if data:
        ...
        bot.send_message(
            message.chat.id,
            f"<b>📊 Твій профіль:</b>\n\n"
            f"🪙 PulseCoins (фарт, бонуси): {data['pulse']}\n"
            f"💰 LuckyTokens (промо, вивід): {data['lucky']}\n"
...
# 2. Оновлена функція збереження з резервною копією
import shutil
...
# 3. Завантаження з резерву якщо основний файл пошкоджений
try:
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, 'r') as f:
            users_data = json.load(f)
